...
mod test {
    ...
    async fn crud_scenario() {
        ...
        let res = repository.find(todo.id).await;
        assert!(res.is_err());
...
pub mod test_utils {
    ...
    mod test {
        ...
        async fn todo_crud_scenario() {
            let text = "handler text".to_string();
            let id = 1;
            ...
            // all
            let todo = repository.all().await.expect("failed get all handler");
            assert_eq!(vec![expected], todo);

            // update
            let text = "update handler text".to_string();
            let todo = repository
                .update(
                    1,
                    UpdateTodo {
